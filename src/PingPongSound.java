...
public class PingPongSound extends Applet implements Runnable, MouseListener,
		MouseMotionListener {
		...
		while (true) {
			...
			if (ballX < 160) {
				if (upwards) {
					...
					if (ballY < batMax)
						batYC = ballY - 5;// (int)(Math.random()*5);
					else
						batYC = batMax;
				}
			}
			...
			if (scoreC == topscore || scoreP == topscore)
				endGame();
	...
	public void paint(Graphics g) {
		...
		if (!started)
			g.drawString(" Click Here to Start! ", 70, 140);

		g.setColor(Color.green);
		if (gameover)
			g.drawString("Game Over! ", 110, 80);
		g.drawString("Computer", 40, 240);
		g.drawString("Player", 200, 240);

		g.setColor(Color.yellow);
		if (gameover)
			g.drawString(whowon, 50, 110);
