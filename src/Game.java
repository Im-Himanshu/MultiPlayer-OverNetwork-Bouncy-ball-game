...
public class Game extends JPanel implements MouseListener, MouseMotionListener,
	...
	private void computerplayer() {
		...
		if (ballx < (x1 + x3) / 2 && openspot[1] == 0 && score[1]<= maxscore) {
		...
		// for player 2
		if (bally > (y1 + y3) / 2 && openspot[2] == 0&& score[2]<= maxscore) {
			if (toLeft && chance2 == 0) {
				int x = error();
				if (x > lengthbat / 2) {
					x = lengthbat / 2 + dia + 1;
		...
		// for player 4
		if (bally < (y1 + y3) / 2 && openspot[4] == 0&& score[4]<= maxscore) {
	...
	public void paint(Graphics g2) {
		...
		if (!isstarted) {
			...
			else g2d.drawString("PLAYER1 :- out" , 30, 30);
			...
			else g2d.drawString("PLAYER2 :- out" , 120, 30);
			
			g2d.setColor(Color.GREEN);
			if(score[1]<maxscore)
				
			g2d.drawString("PLAYER3 :- " + score[3], 210, 30);
			else g2d.drawString("PLAYER3 :- out" , 30, 30);
			...
			else g2d.drawString("PLAYER4 :- out" , 290, 30);
